    ...
    public class FileSearchUtil
    {
        ...
        public IEnumerable<String> EnumerateFiles(string szDriveLetter)
        {
            try
            {
                ...
                do
                {
                    if (DeviceIoControl(m_hCJ, FSCTL_ENUM_USN_DATA, ref mft, Marshal.SizeOf(mft), m_Buffer, m_BufferSize, ref dwRetBytes, IntPtr.Zero))
                    {
                        ...
                        while ((dwRetBytes > 8))
                        {
                            ...
                            bIsFile = !(usnRecord.FileAttribute == FileAttributes.Directory);
                ...
                List<FSNode> fsNodeList = new List<FSNode>();

                foreach (var item in dicFRNLookup.Values)
                {
                    if (item.IsFile)
                    {
                        fsNodeList.Add(item);
                ...
                foreach (FSNode oFSNode in fsNodeList)
                {
        ...
        public IEnumerable<String> EnumerateFiles(DriveInfo drive)
        {
            return EnumerateFiles(drive.Name);
        }

        public string GetPathByFile(DriveInfo drive, string _filename)
        {
            return EnumerateFiles(drive.Name, _filename);
        }

        public List<string> GetPathByFile(string _filename)
        {
            var sw = Stopwatch.StartNew();

            List<string> lolPaths = new List<string>();
            ...
            try
            {
                ...
                foreach (DriveInfo drive in allDrives)
                {
                    ...
                    lolPath = GetPathByFile(drive, _filename);

                    if (!string.IsNullOrEmpty(lolPath))
                    {
                        lolPath = lolPath.Replace(_filename, "");
                        lolPaths.Add(lolPath);
                    }
                }
            }
            catch (Exception exception)
            {
                LogUtil.log("FileSearchUtil.GetPathByFile " + exception.Message);
            }
            finally
            {
                sw.Stop();
            ...
            LogUtil.log(string.Concat("FileSearchUtil.GetPathByFile 全盘扫描耗时： ", sw.ElapsedMilliseconds, " 毫秒，查找文件数量：", lolPaths.Count));

            QueryParam qparam = QueryParam.create("key", "LOLPathSearch");
